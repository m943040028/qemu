                                 ...
                                 uint64_t value)
{
    ...
    A32_BANKED_REG_SET(env, c2_mask, ri->secure,
            ~(((uint32_t)0xffffffffu) >> maskshift));
    A32_BANKED_REG_SET(env, c2_base_mask, ri->secure,
...
static void vmsa_ttbcr_reset(CPUARMState *env, const ARMCPRegInfo *ri)
{
     ...
     */
    A32_BANKED_REG_SET(env, c2_base_mask, ri->secure, 0xffffc000u);
    A32_BANKED_REG_SET(env, c2_mask, ri->secure, 0);
    raw_write(env, ri, 0);
}
...
static const ARMCPRegInfo v7_el3_cp_reginfo[] = {
    ...
    { .name = "SDER", .cp = 15, .crn = 1, .crm = 1, .opc1 = 0, .opc2 = 1,
      .access = PL3_RW, .resetvalue = 0, .secure = 1,
    ...
    { .name = "MVBAR", .cp = 15, .crn = 12, .crm = 0, .opc1 = 0, .opc2 = 1,
      .access = PL3_RW, .writefn = vbar_write, .resetvalue = 0, .secure = 0,
                                   ...
                                   int crm, int opc1, int opc2, int nsbit)
{
    ...
    if (state == ARM_CP_STATE_AA32) {
        if (r->bank_fieldoffsets[0] && r->bank_fieldoffsets[1]) {
            /* Track whether this is a secure or nonsecure instance */
            if (!r2->secure) {
                r2->secure = !nsbit;
            }
             ...
             */
            r2->fieldoffset = r->bank_fieldoffsets[!r2->secure];
